...
namespace TestJSON
{
    ...
    class Program
    {
        ...
        /// 测试序列化对象字段
        /// </summary>
        static void TestSerializeObj()
        {
            var json = "{Name:'lijian'}";
            var stu = JsonConvert.DeserializeObject<Student>(json);
            Console.WriteLine(stu.Name);

            //json字符串存在的属性，实体对象不存在的，也能序列化
            json = "{Name:'lijian',TestName:'zhq'}";
            stu = JsonConvert.DeserializeObject<Student>(json);
            Console.WriteLine(stu.Name);
        }
